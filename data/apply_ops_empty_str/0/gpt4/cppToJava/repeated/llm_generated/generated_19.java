...
This Java code does the same thing as the C++ code. It reads a string from the standard input, processes it with the `lastNonEmptyString` method, and then prints the result to the standard output. The `lastNonEmptyString` method itself also works the same way as in the C++ code, using arrays instead of vectors and a `StringBuilder` instead of a `string` for building the result string.